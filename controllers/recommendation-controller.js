...
const fetchAllAnimes = async (geminiRecommendations) => {
    try {
        const response = await Promise.allSettled(geminiRecommendations.map(async (anime) => {
            try{
                const animeData = await fetchAnimeFromJikanByName(anime.title);
                return animeData ? {...animeData, similarity_reasons: anime.similarity_reasons} : null;
            } catch(error) {
                console.error(`Error fetching anime with title: ${anime.title}`, error);
                return null;
            }
         }));
...
const fetchAnimeFromJikanByName = async (title) => {
    try {
        ...
        const animeResponse = await limiter.schedule(() => axios.get(`${jikanUrl}/?q=${title}`));
